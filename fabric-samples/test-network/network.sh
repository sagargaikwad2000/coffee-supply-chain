...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Checker Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-checker.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating Exporter Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-exporter.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating Importer Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-importer.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  infoln "Generating CCP files for Producer and Processor"
...
# call the script to create the channel, join the peers of producer and processor,
...
function networkDown() {
  ...
  # stop checker containers also in addition to producer and processor, in case we were running sample to add checker
  COMPOSE_CHECKER_BASE_FILES="-f addChecker/compose/${COMPOSE_FILE_CHECKER_BASE} -f addChecker/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_CHECKER_BASE}"
  COMPOSE_CHECKER_COUCH_FILES="-f addChecker/compose/${COMPOSE_FILE_CHECKER_COUCH} -f addChecker/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_CHECKER_COUCH}"
  COMPOSE_CHECKER_CA_FILES="-f addChecker/compose/${COMPOSE_FILE_CHECKER_CA} -f addChecker/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_CHECKER_CA}"
  COMPOSE_CHECKER_FILES="${COMPOSE_CHECKER_BASE_FILES} ${COMPOSE_CHECKER_COUCH_FILES} ${COMPOSE_CHECKER_CA_FILES}"

  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_CHECKER_FILES} down --volumes --remove-orphans
  elif [ "${CONTAINER_CLI}" == "podman" ]; then
    ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_CHECKER_FILES} down --volumes
  else
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.producer.example.com docker_peer0.processor.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf addChecker/fabric-ca/checker/msp addChecker/fabric-ca/checker/tls-cert.pem addChecker/fabric-ca/checker/ca-cert.pem addChecker/fabric-ca/checker/IssuerPublicKey addChecker/fabric-ca/checker/IssuerRevocationPublicKey addChecker/fabric-ca/checker/fabric-ca-server.db'
...
# use this as the default docker-compose yaml definition for checker
COMPOSE_FILE_CHECKER_BASE=compose-checker.yaml
# use this as the docker compose couch file for checker
COMPOSE_FILE_CHECKER_COUCH=compose-couch-checker.yaml
# certificate authorities compose file
COMPOSE_FILE_CHECKER_CA=compose-ca-checker.yaml
#
